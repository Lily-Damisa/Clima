...
function Weather() {
  ...
  async function weatherData(e){
    ...
    const data = await fetch(
      `https://api.openweathermap.org/data/2.5/weatherq=${form.city},${form.country}&appid=${myApiKey}`
      );
    
    if (form.city == ""){
      alert("Abeg, type something na. Don't type your village name o!");
    } 
  ...
  return (
    <div>
        ...
        <form>
          ...
          <button className='text-white rounded-full text-black text-slate-500 bg-blue-500 hover:bg-blue-300 px-5 py-2' onClick={(e) => weatherData(e)}>Search</button>
        </form>
